...
from pyui.base import Size, Color, Position, get_asset
...
def screen1():
    ...
    return Frame(None, widget=box)


def screen2():
    # same as above, just in a Border with some better margins
    box = HBox(widgets=[ColorRect(Size(50, 50), Color.BLUE), TextLabel('Hello, World')])
    return Border(None, pos=Position(100, 100), widget=box)


def screen3():
    # now replace the colorrect with an image
    box = HBox(widgets=[Image(get_asset('icons/open.png')), TextLabel('Hello, World')])
    return Border(None, Position(100, 100), widget=box)


def screen4():
    # menu items in a VBox, very similar to our menu
    r1 = HBox(widgets=[Image(get_asset('icons/open.png')), TextLabel('Hello, World')])
    r2 = HBox(widgets=[Image(get_asset('icons/open.png')), TextLabel('Another one')])
    r3 = HBox(widgets=[Image(get_asset('icons/open.png')), TextLabel('This to end!')])
    box = VBox(widgets=[r1, r2, r3])
    return Frame(None, pos=Position(100, 100), widget=box)
...
def screen5():
    ...
    return Menu(pos=Position(100, 100), items=[menu1, menu2, menu3])
